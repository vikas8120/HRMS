...
function HrBreadcrumb() {
  ...
  const filteredParts = parts.filter((part) => part !== 'hr')

  const crumbItems = filteredParts.map((part, index) => {
    const to = `/hr/${filteredParts.slice(0, index + 1).join('/')}`
    return { label: prettify(part), to, isLast: index === filteredParts.length - 1 }
  })

  return (
    <div className="breadcrumb" aria-label="Breadcrumb">
      <Link to="/hr/dashboard" className="breadcrumb-home"><Home size={14} /> Home</Link>
      {crumbItems.map((item) => (
