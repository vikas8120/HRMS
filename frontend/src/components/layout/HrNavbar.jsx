...
function HrNavbar() {
  ...
  const results = useMemo(() => {
    ...
    return hrSearchItems
      .filter((item) => `${item.title} ${item.subtitle}`.toLowerCase().includes(value))
      .slice(0, 10)
  }, [query])

  const activeLabel = useMemo(() => {
    const active = hrSearchItems.find((item) => pathname === item.route || pathname.startsWith(`${item.route}/`))
    return active?.title || 'HR Workspace'
  }, [pathname])
