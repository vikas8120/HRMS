...
function GlobalUsersModulePage({ page }) {
  ...
  const runBulkImport = async () => {
    if (!bulkImportText.trim()) return toastError('Paste a JSON array of users to import')
    try {
      const parsed = JSON.parse(bulkImportText)
      if (!Array.isArray(parsed)) return toastError('Bulk import payload must be an array')
      const res = await bulkImportUsers(parsed)
      toastOk(`Imported ${res.count || 0} users`)
      setBulkModalOpen(false)
      setBulkImportText('')
      loadUsers()
    } catch (error) {
      toastError(error?.response?.data?.message || 'Invalid JSON or import failed')
    }
  }
  ...
  const renderStatusActions = () => (
    <div className="panel"><h3>User Status Actions</h3><DataTable columns={userCols} rows={userRows} showActions={false} /><div className="actions-row">{userRows.slice(0,5).map((u) => <div key={u.id} className="inline-action-card"><span>{u.name}</span><div className="actions-row"><Button variant="danger" onClick={async () => { try { const res = await updateGlobalUserStatus(u.id, 'blocked'); toastOk(res?.message || 'Blocked'); loadUsers() } catch (error) { toastError(error?.response?.data?.message || 'Failed to block user') } }}>Block</Button><Button onClick={async () => { try { const res = await updateGlobalUserStatus(u.id, 'active'); toastOk(res?.message || 'Unlocked'); loadUsers() } catch (error) { toastError(error?.response?.data?.message || 'Failed to unlock user') } }}>Unlock</Button><Button variant="ghost" onClick={async () => { try { const res = await forceLogoutUser(u.id); toastOk(res?.message || 'Forced logout') } catch (error) { toastError(error?.response?.data?.message || 'Failed to force logout') } }}>Force Logout</Button></div></div>)}</div></div>
  )

  const renderLoginHistory = () => (
    <div className="panel"><h3>User Login History</h3><DataTable columns={loginCols} rows={loginRows} showActions={false} /></div>
  )

  const renderActiveSessions = () => (
    <div className="panel"><h3>Active Sessions</h3><DataTable columns={sessionCols} rows={sessionRows} showActions={false} /></div>
  )

  const renderFailedAttempts = () => (
    <div className="panel"><h3>Failed Login Attempts</h3><DataTable columns={loginCols} rows={loginRows.filter((x) => x.success === 'failed')} showActions={false} /></div>
  )

  const renderDeviceTracking = () => (
    <div className="panel"><h3>User Device Tracking</h3><DataTable columns={deviceCols} rows={deviceRows} showActions={false} /></div>
  )
