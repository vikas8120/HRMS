...
function SupportCenterModulePage({ page }) {
  ...
  const renderTicketTable = () => (
    <>
      ...
      <div className="panel"><div className="panel-head"><h3>{page}</h3><Button onClick={() => { setSelectedTicketId(''); setTicketModal(true) }}>Create Ticket</Button></div><DataTable columns={cols} rows={filteredRows} onView={(row) => setSelectedTicketId(row.id)} onEdit={openEdit} onDelete={() => {}} /><div className="pagination-row"><Button variant="ghost" disabled={pagination.page <= 1} onClick={() => setPagination((p) => ({ ...p, page: p.page - 1 }))}>Prev</Button><span>Page {pagination.page} of {pagination.totalPages || 1}</span><Button variant="ghost" disabled={pagination.page >= pagination.totalPages} onClick={() => setPagination((p) => ({ ...p, page: p.page + 1 }))}>Next</Button></div></div>
    </>
  )

  const renderAssignPrioritySla = () => (
    <div className="panel"><h3>{page}</h3><DataTable columns={cols} rows={rows} onView={(row) => setSelectedTicketId(row.id)} onEdit={() => {}} onDelete={() => {}} /><div className="actions-row">{rows.slice(0, 6).map((r) => <div key={r.id} className="inline-action-card"><span>{r.ticketNo}</span><div className="actions-row"><Button variant="ghost" onClick={async () => { const agent = agents[0]?._id || null; await assignSupportTicket(r.id, agent); toastOk('Assigned'); loadBase() }}>Assign</Button><Button variant="ghost" onClick={async () => { await updateTicketPriority(r.id, 'critical'); toastOk('Priority updated'); loadBase() }}>Set Critical</Button><Button variant="ghost" onClick={async () => { const due = new Date(Date.now() + 24*60*60*1000).toISOString(); await updateTicketSla(r.id, due); toastOk('SLA updated'); loadBase() }}>Set SLA +24h</Button></div></div>)}</div></div>
  )
      ...
      <div className="panel">
        ...
        {!selectedTicketId ? <EmptyState title="Select a ticket to open chat" /> : (
          <>
            ...
            <div className="actions-row"><FormInput label="Message" value={messageText} onChange={(e) => setMessageText(e.target.value)} /><Button onClick={async () => { if (!messageText) return; await addTicketMessage(selectedTicketId, { senderType: 'admin', senderName: 'Super Admin', message: messageText }); setMessageText(''); const r = await listTicketMessages(selectedTicketId); setMessages(r.items || []); toastOk('Message sent') }}>Send</Button></div>
            <div className="actions-row"><FormInput label="Internal Note" value={noteText} onChange={(e) => setNoteText(e.target.value)} /><Button variant="ghost" onClick={async () => { if (!noteText) return; await addInternalNote(selectedTicketId, noteText, 'Super Admin'); setNoteText(''); toastOk('Note added'); loadBase() }}>Add Note</Button></div>
            <Button variant="primary" onClick={async () => { await resolveSupportTicket(selectedTicketId, 'Resolved from support center'); toastOk('Ticket resolved'); loadBase() }}>Resolve Ticket</Button>
  ...
  const renderCategoryAgent = (kind) => (
    <div className="panel"><h3>{kind === 'cat' ? 'Ticket Categories' : 'Support Agents'}</h3><div className="actions-row"><Button onClick={async () => { if (kind === 'cat') await createTicketCategory({ name: `Category ${Date.now()}` }); else await createSupportAgent({ name: `Agent ${Date.now()}`, email: `agent_${Date.now()}@hrms.com` }); toastOk('Created'); loadBase() }}>Add {kind === 'cat' ? 'Category' : 'Agent'}</Button></div><DataTable columns={kind === 'cat' ? [{ key: 'name', label: 'Name' }, { key: 'description', label: 'Description' }, { key: 'slaHours', label: 'SLA Hours' }] : [{ key: 'name', label: 'Name' }, { key: 'email', label: 'Email' }, { key: 'level', label: 'Level' }, { key: 'active', label: 'Status' }]} rows={(kind === 'cat' ? categories : agents).map((x) => ({ id: x._id, name: x.name, description: x.description || '-', slaHours: x.slaHours || '-', email: x.email || '-', level: x.level || '-', active: x.active ? 'active' : 'inactive' }))} onView={() => {}} onEdit={() => {}} onDelete={() => {}} /></div>
  )
