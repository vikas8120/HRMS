...
function DashboardModulePage({ type, title }) {
  ...
  const cards = useMemo(() => {
    ...
    if (type === 'system-health-status') return [['API Status', payload.apiStatus], ['Database Status', payload.databaseStatus], ['Storage Status', payload.storageStatus], ['Uptime', payload.uptime], ['Last Checked', payload.lastCheckedAt ? new Date(payload.lastCheckedAt).toLocaleString() : '-']]
    return []
  }, [payload, type])

  const tableConfig = useMemo(() => {
    if (!payload) return { columns: [], rows: [] }
    if (type === 'platform-overview') return { columns: [{ key: 'id', label: 'ID' }, { key: 'actor', label: 'Actor' }, { key: 'action', label: 'Action' }, { key: 'module', label: 'Module' }, { key: 'severity', label: 'Severity' }, { key: 'time', label: 'Time' }], rows: (payload.recentActivities || []).map((a) => ({ ...a, time: a.time ? new Date(a.time).toLocaleString() : '-' })) }
    if (type === 'total-companies') return { columns: [{ key: 'companyName', label: 'Company Name' }, { key: 'code', label: 'Code' }, { key: 'industry', label: 'Industry' }, { key: 'status', label: 'Status' }, { key: 'createdAt', label: 'Created Date' }], rows: (payload.items || []).map((r) => ({ ...r, createdAt: r.createdAt ? new Date(r.createdAt).toLocaleDateString() : '-' })) }
    if (type === 'active-users') return { columns: [{ key: 'name', label: 'Name' }, { key: 'email', label: 'Email' }, { key: 'role', label: 'Role' }, { key: 'company', label: 'Company' }, { key: 'status', label: 'Status' }, { key: 'lastLogin', label: 'Last Login' }], rows: (payload.items || []).map((r) => ({ ...r, lastLogin: r.lastLogin ? new Date(r.lastLogin).toLocaleString() : '-' })) }
    if (type === 'active-subscriptions') return { columns: [{ key: 'company', label: 'Company' }, { key: 'plan', label: 'Plan' }, { key: 'status', label: 'Status' }, { key: 'startDate', label: 'Start Date' }, { key: 'endDate', label: 'End Date' }, { key: 'autoRenew', label: 'Auto Renew' }], rows: (payload.items || []).map((r) => ({ ...r, startDate: r.startDate ? new Date(r.startDate).toLocaleDateString() : '-', endDate: r.endDate ? new Date(r.endDate).toLocaleDateString() : '-', autoRenew: r.autoRenew ? 'Yes' : 'No' })) }
    if (type === 'monthly-revenue') return { columns: [{ key: 'invoiceNo', label: 'Invoice No' }, { key: 'company', label: 'Company' }, { key: 'amount', label: 'Amount' }, { key: 'status', label: 'Status' }, { key: 'date', label: 'Date' }], rows: (payload.items || []).map((r) => ({ ...r, amount: Number(r.amount || 0).toFixed(2), date: r.date ? new Date(r.date).toLocaleDateString() : '-' })) }
    if (type === 'support-ticket-summary') return { columns: [{ key: 'ticketId', label: 'Ticket ID' }, { key: 'subject', label: 'Subject' }, { key: 'company', label: 'Company' }, { key: 'priority', label: 'Priority' }, { key: 'status', label: 'Status' }, { key: 'createdAt', label: 'Created Date' }], rows: (payload.items || []).map((r) => ({ ...r, createdAt: r.createdAt ? new Date(r.createdAt).toLocaleDateString() : '-' })) }
    if (type === 'system-health-status') return { columns: [{ key: 'status', label: 'Status' }, { key: 'apiStatus', label: 'API Status' }, { key: 'databaseStatus', label: 'Database Status' }, { key: 'storageStatus', label: 'Storage Status' }, { key: 'uptime', label: 'Uptime' }, { key: 'createdAt', label: 'Checked At' }], rows: (payload.logs || []).map((r) => ({ ...r, createdAt: r.createdAt ? new Date(r.createdAt).toLocaleString() : '-' })) }
    if (type === 'recent-activities') return { columns: [{ key: 'id', label: 'ID' }, { key: 'actor', label: 'Actor' }, { key: 'actorRole', label: 'Role' }, { key: 'module', label: 'Module' }, { key: 'action', label: 'Action' }, { key: 'description', label: 'Description' }, { key: 'severity', label: 'Severity' }, { key: 'time', label: 'Time' }], rows: (payload.items || []).map((r) => ({ ...r, time: r.time ? new Date(r.time).toLocaleString() : '-' })) }
    return { columns: [], rows: [] }
  }, [payload, type])
