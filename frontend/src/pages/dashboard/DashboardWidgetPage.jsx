...
const shortCode = (sectionKey) => sectionKey.replace(/-/g, '').slice(0, 4).toUpperCase()

function DashboardWidgetPage({ title, sectionKey }) {
  ...
  const loadData = async () => {
    ...
    setError('')
    try {
      const data = await listDashboardWidgets(sectionKey, { page: pagination.page, limit: pagination.limit, search, status: statusFilter })
  ...
  }, [pagination.page, search, statusFilter, sectionKey])

  const tableRows = useMemo(
    () =>
      records.map((entry, index) => ({
        id: entry._id,
        idLabel: `${shortCode(sectionKey)}-${index + 1}`,
        name: entry.name,
        status: entry.status,
        owner: entry.owner,
  ...
  const openEdit = async (row) => {
    ...
    setOpen(true)
    try {
      const data = await getDashboardWidgetById(sectionKey, row.id)
  ...
  const onSubmit = async (event) => {
    ...
    try {
      if (mode === 'Add') await createDashboardWidget(sectionKey, form)
      else await updateDashboardWidget(sectionKey, selected._id || selected.id, form)
      setOpen(false)
      setSelected(null)
      await loadData()
    } catch (err) {
  ...
  const onDelete = async () => {
    if (!selected) return
    try {
      await deleteDashboardWidget(sectionKey, selected._id || selected.id)
  ...
  return (
    <section className="section-layout">
      <PageHeader
        title={title}
        description={`${title} workspace with search, filters, data table, and CRUD-ready actions.`}
        breadcrumb={['Super Admin', 'Dashboard', title]}
        primaryActionLabel={`Add ${title}`}
      ...
      <Modal open={open} title={`${mode} ${title}`} onClose={() => setOpen(false)}>
      ...
      <Modal open={viewOpen} title={`View ${title}`} onClose={() => setViewOpen(false)}>
        {selected ? (
